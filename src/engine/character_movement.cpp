...
void TA_Character::updateGround() {
    // ── Spindash ─────────────────────────────────────────────────────────────
    if(!hurt && !remoteRobot) {
        bool downHeld = (links.controller->getDirection() == TA_DIRECTION_DOWN);
        bool aJust    = links.controller->isJustPressed(TA_BUTTON_A);

        if(spindashCharging) {
            if(aJust) {
                // Each A press = one rev, adds speed
                spindashRevs++;
                spindashRevs = std::min(spindashRevs, (int)spindashMaxCharge);
                spindashSpeed = spindashMinSpeed + 
                    (spindashTopSpeed - spindashMinSpeed) * (spindashRevs / spindashMaxCharge);
                TA::gamepad::rumble(0.3f + spindashRevs * 0.15f, 0.2f, 6);
                spindashRevSound.play();
            }
            if(!downHeld) {
                // Released down — launch!
                spindashCharging = false;
                crouch = false;
                if(spindashSpeed > 0) {
                    state = STATE_SPINDASH;
                    spindashGoSound.play();
                    TA::gamepad::rumble(0.9f, 0.5f, 12);
                }
                return;
            }
            // Still charging — stay crouched, no movement
            velocity.x = 0;
            jump = spring = false;
            coyoteTime = 0;
            crouch = true;
        ...
        // Start charging: hold down then press A
        if(downHeld && aJust) {
            initSpindash();
            return;
        }
