...
void TA_Character::load(TA_Links newLinks) {
    ...
    spindashRevSound.load("sound/land.ogg", TA_SOUND_CHANNEL_SFX2);
    spindashGoSound.load("sound/jump.ogg", TA_SOUND_CHANNEL_SFX2);
...
void TA_Character::updateAnimation() {
    // Spindash states override everything
    if(state == STATE_SPINDASH) {
        setAnimation("spindash_roll");
        return;
    }
    if(spindashCharging) {
        setAnimation("spindash_charge");
        return;
    }
    if(rings >= 0 && hurt) {
        setAlpha(255);
...
void TA_Character::initSpindash() {
...
void TA_Character::updateSpindash() {
    crouch = false;

    // Apply horizontal dash speed
    velocity.x = (flip ? -1.0f : 1.0f) * spindashSpeed;

    // Decay speed each frame
    spindashSpeed -= spindashDecay * TA::elapsedTime;

    // Gravity applies normally while dashing
    if(!ground) {
        velocity.y += grv * TA::elapsedTime;
        velocity.y = std::min(velocity.y, maxJumpSpeed);
    }

    // Update spindash hitbox — covers Tails' full rolled-up body
    // Active for the entire duration of the dash (same lifetime as momentum)
    spindashHitbox.setRectangle(TA_Point(14, 18), TA_Point(34, 38));
    spindashHitbox.setPosition(position);

    // End dash when speed runs out or hits a wall
    if(spindashSpeed <= spindashMinSpeed || wall) {
        state = STATE_NORMAL;
        spindashSpeed = 0;
        velocity.x *= 0.3f;
        // Clear hitbox when dash ends
        spindashHitbox.setRectangle(TA_Point(0, 0), TA_Point(0, 0));
    }
